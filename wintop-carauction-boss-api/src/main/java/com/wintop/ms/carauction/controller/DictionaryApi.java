...
public class DictionaryApi {
    ...
    /**查询车辆基本信息所用到的字典基础数据*/
    String findForAutoBaseInfo_Url = Constants.ROOT + "/dictionary/findForAutoBaseInfo";
    ...
    public ResponseEntity<ResultModel> getByPCode(@RequestBody JSONObject obj) {
    ...
    @PostMapping(value = "/getForAutoBaseInfo")
    @ApiOperation(value = "查询车辆基础数据相关的字典集合")
    public ResponseEntity<ResultModel> getForAutoBaseInfo() {
        logger.info("查询车辆基础数据相关的字典集合");

        Map map = new HashMap();
        map.put("status","1");
        ResponseEntity<JSONObject> resultResponseEntity = this.restTemplate.exchange(
                RequestEntity
                        .post(URI.create(findForAutoBaseInfo_Url))
                        .contentType(MediaType.APPLICATION_JSON)
                        .body(map),JSONObject.class);
        JSONObject serviceResult = resultResponseEntity.getBody();
        //1、先判断返回网络状态码是否成功
        if(resultResponseEntity.getStatusCode()== HttpStatus.OK){
            ...
            if (serviceResult.get("success").toString()=="true") {
                ...
                if (object!=null){//已存在
                    ...
                    resultModel = ResultModel.error(ResultStatus.OBJECT_NOT_EXIST);
                }
                return new ResponseEntity<>(resultModel, HttpStatus.OK);
            }else {//异常情况
