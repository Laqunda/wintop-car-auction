...
public class CarAutoConfApi {
    ...
    public ResponseEntity<ResultModel> getAutoConfBase(@RequestParam Long autoId){
        Map map = new HashMap();
    ...
    @AuthUserToken
    public ResponseEntity<ResultModel> saveOption(@ApiParam(value = "车辆配置json，车辆id+配置json",required = true) @RequestBody JSONObject object){
        ResponseEntity<JSONObject> response=this.restTemplate.exchange(
                RequestEntity
